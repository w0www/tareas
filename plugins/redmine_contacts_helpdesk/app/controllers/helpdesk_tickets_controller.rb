class HelpdeskTicketsController < ApplicationController
  ...
  def update
    ...
    @helpdesk_ticket.customer = Contact.find(params[:helpdesk_ticket][:contact_id]) if params[:helpdesk_ticket][:contact_id]
...
private 
  ...
  def find_issue
    ...
    @helpdesk_ticket = @issue.helpdesk_ticket ? @issue.helpdesk_ticket : HelpdeskTicket.new(:ticket_date => Time.now, :issue => @issue)
  rescue ActiveRecord::RecordNotFound
    render_404
  end
